...
mode = 'nonlinear'
v2 = True # set to true if you want to compute jacobian along the way in linear sys
setGains = True
...
t_kf = time_dilation * np.array([1,spup,0.3,0.7,spup,0.7,0.5,0.8], dtype=np.float64)
...
all_est_path_asim = np.zeros((total_steps-1,n))
...
# plt.plot(all_est_path_asim[:,5], label = 'airsim_yaw')
# plt.plot(all_diff_flat_states[:,5], label = 'DF yaw')
plt.legend()

plt.figure()
plt.plot(all_est_path[:,3], label = 'actual roll')
plt.plot(all_diff_flat_states[:,3], label = 'nominal roll')
plt.plot(all_est_path[:,4], label = 'actual pitch')
plt.plot(all_diff_flat_states[:,4], label = 'nominal pitch')
plt.legend()
